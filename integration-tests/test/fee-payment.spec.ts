...
import { predeploys, getContractFactory } from '@mantlenetworkio/contracts'
...
const setPrices = async (env: MantleEnv, value: number | BigNumber) => {
  ...
  await gasPrice.wait()
  const baseFee = await env.messenger.contracts.l2.BVM_GasPriceOracle.connect(
    gasPriceOracleWallet
  ).setL1BaseFee(value)
  await baseFee.wait()
}
...
describe('Fee Payment Integration Tests', async () => {
  ...
  hardhatTest(
    `should return eth_gasPrice equal to BVM_GasPriceOracle.gasPrice`,
    async () => {
      const assertGasPrice = async () => {
        const gasPrice = await env.l2Wallet.getGasPrice()
        const oracleGasPrice =
          await env.messenger.contracts.l2.BVM_GasPriceOracle.connect(
            gasPriceOracleWallet
          ).gasPrice()
      ...
      const tx = await env.messenger.contracts.l2.BVM_GasPriceOracle.connect(
        gasPriceOracleWallet
      ).setGasPrice(1000)
      await tx.wait()
  ...
  hardhatTest('Paying a nonzero but acceptable gasPrice fee', async () => {
    ...
    const l1Fee = await env.messenger.contracts.l2.BVM_GasPriceOracle.connect(
      gasPriceOracleWallet
    ).getL1Fee(raw)
  ...
  hardhatTest('should compute correct fee', async () => {
    ...
    const BVM_GasPriceOracle = getContractFactory('BVM_GasPriceOracle')
      .attach(predeploys.BVM_GasPriceOracle)
      .connect(env.l2Wallet)

    const WETH = getContractFactory('BVM_ETH')
      .attach(predeploys.BVM_ETH)
      .connect(env.l2Wallet)
    ...
    const l1Fee = await BVM_GasPriceOracle.connect(
      gasPriceOracleWallet
    ).getL1Fee(raw)
    ...
    const feeVaultAfter = await WETH.balanceOf(predeploys.BVM_SequencerFeeVault)
  ...
  it('should not be able to withdraw fees before the minimum is met', async () => {
    await expect(env.messenger.contracts.l2.BVM_SequencerFeeVault.withdraw()).to
      .be.rejected
  })
  ...
  hardhatTest(
    ...
    async () => {
      const l1FeeWallet =
        await env.messenger.contracts.l2.BVM_SequencerFeeVault.l1FeeWallet()
      const balanceBefore = await env.l1Wallet.provider.getBalance(l1FeeWallet)
      const withdrawalAmount =
        await env.messenger.contracts.l2.BVM_SequencerFeeVault.MIN_WITHDRAWAL_AMOUNT()

      // Transfer the minimum required to withdraw.
      const tx = await env.l2Wallet.sendTransaction({
        to: env.messenger.contracts.l2.BVM_SequencerFeeVault.address,

...
type peer struct {
	...
	*p2p.Peer
	...
	knowRollbackMsg          mapset.Set                       // Set of end msg index to be known by this peer
...
func (ps *peerSet) PeersWithoutRollbackMsg(msgHash common.Hash) []*peer {
	...
	for _, p := range ps.peers {
		if !p.knowRollbackMsg.Contains(msgHash) {
			list = append(list, p)
		}
	}
	return list

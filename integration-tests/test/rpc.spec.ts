/* Imports: External */
import { expectApprox, getChainId, sleep } from '@mantleio/mt-core-utils'
...
describe('Basic RPC tests', () => {
  ...
  describe('eth_sendRawTransaction', () => {
    ...
    it('should reject a transaction with too low of a fee', async () => {
      ...
      let gasPrice
      if (isHH) {
        gasPrice = await env.messenger.contracts.l2.BVM_GasPriceOracle.connect(
          gasPriceOracleWallet
        ).gasPrice()
  ...
  describe('eth_getBalance', () => {
    it('should get the BVM_ETH balance', async () => {
      const rpcBalance = await env.l2Provider.getBalance(env.l2Wallet.address)
      const contractBalance =
        await env.messenger.contracts.l2.BVM_ETH.balanceOf(env.l2Wallet.address)

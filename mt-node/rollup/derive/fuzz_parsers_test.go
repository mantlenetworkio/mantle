...
var (
	...
	portalContract, _      = bindings.NewMantlePortal(addr, nil)
...
func FuzzUnmarshallLogEvent(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, _to, _mint, _value, data []byte, l2GasLimit uint64, isCreation bool) {
		...
		state.SetCode(addr, common.FromHex(bindings.MantlePortalDeployedBin))
		...
		reconstructed := &bindings.MantlePortalTransactionDeposited{
			From:       dep.From,
			Version:    common.Big0,
		...
		inputArgs := &bindings.MantlePortalTransactionDeposited{
			From:       from,
			To:         to,
			Version:    common.Big0,
			OpaqueData: opaqueData,

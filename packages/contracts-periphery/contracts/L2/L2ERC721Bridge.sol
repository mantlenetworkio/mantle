...
import {
    CrossDomainEnabled
} from "@bitdaoio/contracts/contracts/libraries/bridge/CrossDomainEnabled.sol";
import {
    OwnableUpgradeable
...
import { IBitnetworkMintableERC721 } from "../universal/op-erc721/IBitnetworkMintableERC721.sol";
import { Semver } from "../universal/Semver.sol";

/**
 * @title L2ERC721Bridge
 * @notice The L2 ERC721 bridge is a contract which works together with the L1 ERC721 bridge to
 *         make it possible to transfer ERC721 tokens between Bitnetwork and Ethereum. This contract
...
contract L2ERC721Bridge is Semver, CrossDomainEnabled, OwnableUpgradeable {
    ...
    function finalizeBridgeERC721(
        ...
        if (
            ...
            ERC165Checker.supportsInterface(
                _localToken,
                type(IBitnetworkMintableERC721).interfaceId
            ) && _remoteToken == IBitnetworkMintableERC721(_localToken).remoteToken()
        ) {
            // When a deposit is finalized, we give the NFT with the same tokenId to the account
            // on L2.
            // slither-disable-next-line reentrancy-events
            IBitnetworkMintableERC721(_localToken).mint(_to, _tokenId);
            // slither-disable-next-line reentrancy-events
    ...
    function _initiateBridgeERC721(
        ...
        require(
            _from == IBitnetworkMintableERC721(_localToken).ownerOf(_tokenId),
        ...
        // usage
        // slither-disable-next-line reentrancy-events
        IBitnetworkMintableERC721(_localToken).burn(_from, _tokenId);

        // Construct calldata for l1ERC721Bridge.finalizeBridgeERC721(_to, _tokenId)
        // slither-disable-next-line reentrancy-events
        address remoteToken = IBitnetworkMintableERC721(_localToken).remoteToken();
        require(

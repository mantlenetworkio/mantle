...
describe('BitnetworkMintableERC721', () => {
  ...
  before(async () => {
    ...
    BitnetworkMintableERC721 = await (
      await ethers.getContractFactory('BitnetworkMintableERC721')
    ).deploy(
    ...
    await BitnetworkMintableERC721.connect(l2BridgeImpersonator).mint(
      aliceAddress,
      TOKEN_ID,
      {
  ...
  describe('mint and burn', () => {
    it('should not allow anyone but the L2 bridge to mint and burn', async () => {
      await expect(
        BitnetworkMintableERC721.connect(alice).mint(aliceAddress, 100)
      ).to.be.revertedWith(
        'BitnetworkMintableERC721: only bridge can call this function'
      )
      await expect(
        BitnetworkMintableERC721.connect(alice).burn(aliceAddress, 100)
      ).to.be.revertedWith(
        'BitnetworkMintableERC721: only bridge can call this function'
      )
    })
  ...
  describe('supportsInterface', () => {
    it('should return the correct interface support', async () => {
      // ERC165
      expect(await BitnetworkMintableERC721.supportsInterface(0x01ffc9a7)).to.be
        .true

      // BitnetworkMintableERC721
      expect(await BitnetworkMintableERC721.supportsInterface(0x051e4975)).to.be
        .true

      // ERC721
      expect(await BitnetworkMintableERC721.supportsInterface(0x80ac58cd)).to.be
        .true

      // Some bad interface
      expect(await BitnetworkMintableERC721.supportsInterface(0xffffffff)).to.be
        .false
    })
  })
  ...
  describe('tokenURI', () => {
    it('should return the correct token uri', async () => {
      ...
      expect(await BitnetworkMintableERC721.tokenURI(TOKEN_ID)).to.equal(tokenUri)
    })
  })
})

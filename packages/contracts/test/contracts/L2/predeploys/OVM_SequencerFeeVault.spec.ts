...
describe('BVM_SequencerFeeVault', () => {
  ...
  let BVM_SequencerFeeVault: Contract
  beforeEach(async () => {
    const factory = await hre.ethers.getContractFactory('BVM_SequencerFeeVault')
    BVM_SequencerFeeVault = await factory.deploy(await signer1.getAddress())
  })

  describe('withdraw', async () => {
    it('should fail if the contract does not have more than the minimum balance', async () => {
      await expect(BVM_SequencerFeeVault.withdraw()).to.be.reverted
    })

    it('should succeed when the contract has exactly sufficient balance', async () => {
      // Send just the balance that the contract needs.
      const amount = await BVM_SequencerFeeVault.MIN_WITHDRAWAL_AMOUNT()

      await signer1.sendTransaction({
        to: BVM_SequencerFeeVault.address,
        value: amount,
      })

      await expect(BVM_SequencerFeeVault.withdraw()).to.not.be.reverted

      expect(
        Fake__L2StandardBridge.withdrawTo.getCall(0).args[0]
      ).to.deep.equal(predeploys.BVM_ETH)
      expect(
    ...
    it('should succeed when the contract has more than sufficient balance', async () => {
      ...
      let amount = await BVM_SequencerFeeVault.MIN_WITHDRAWAL_AMOUNT()
      amount = amount.mul(2)
    ...
    it('should have an owner in storage slot 0x00...00', async () => {
      ...
      BVM_SequencerFeeVault = await factory.deploy(`0x${'11'.repeat(20)}`)

      expect(
        await hre.ethers.provider.getStorageAt(
          BVM_SequencerFeeVault.address,

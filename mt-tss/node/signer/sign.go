...
import (
	...
	"github.com/mantlenetworkio/mantle/mt-tss/node/tsslib/keysign"
	...
	go func() {
		...
		for {
			...
			case req := <-p.signRequestChan:
				...
				var requestBody tsscommon.SignOutputRequest
...
func (p *Processor) SignGo(resId tdtypes.JSONRPCStringID, sign tsscommon.NodeSignRequest, logger zerolog.Logger) error {
	var data []byte
	requestBody := sign.RequestBody.(tsscommon.SignOutputRequest)
	...
	if signByte == nil {
		signData, culprits, err := p.handleSign(sign, hash, logger)
		if err != nil {
			logger.Error().Msgf(" %s sign failed ", hashStr)
			var errorRes tdtypes.RPCResponse
			if len(culprits) > 0 {
				respData := strings.Join(culprits, ",")
				errorRes = tdtypes.NewRPCErrorResponse(resId, tsscommon.CulpritErrorCode, err.Error(), respData)
	...
	if err != nil {
		...
		err := p.storeOutput(requestBody.ElectionId, requestBody.OutputRoot, sign.Nodes, sign.ClusterPublicKey)
		if err != nil {
...
func (p *Processor) checkMessages(sign tsscommon.SignOutputRequest) (err error, hashByte, signByte []byte) {
...
func signMsgToHash(msg tsscommon.SignOutputRequest) ([]byte, error) {
	l1BlockNumber := new(big.Int).SetUint64(msg.L2BlockNumber)
	l2BlockNumber := new(big.Int).SetUint64(msg.L2BlockNumber)

	return tsscommon.StateBatchHash(msg.OutputRoot, l2BlockNumber, ethcommon.HexToHash(msg.L1BlockHash), l1BlockNumber)
}
...
func (p *Processor) storeOutput(electionId uint64, outputRoot [32]byte, workingNodes []string, poolPubkey string) error {
	...
	if err = p.nodeStore.SetOutput(sbi); err != nil {
		return err
	}
	return nil
}

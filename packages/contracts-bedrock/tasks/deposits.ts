...
task('deposit', 'Deposits funds onto L2.')
  ...
  .setAction(async (args, hre) => {
    ...
    const proxy = await hre.deployments.get('MantlePortalProxy')

    const MantlePortal = await hre.ethers.getContractAt(
      'MantlePortal',
      proxy.address
    )
    ...
    const tx = await MantlePortal.depositTransaction(
      to,
      amountWei,
      '3000000',
      false,

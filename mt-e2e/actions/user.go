...
func NewL1Bindings(t Testing, l1Cl *ethclient.Client, deployments *e2eutils.DeploymentsL1) *L1Bindings {
	MantlePortal, err := bindings.NewMantlePortal(deployments.MantlePortalProxy, l1Cl)
...
func (s *CrossLayerUser) ActDeposit(t Testing) {
	...
	tx, err := s.L1.env.Bindings.MantlePortal.DepositTransaction(&s.L1.txOpts, toAddr, depositTransferValue, depositGas, isCreation, s.L2.txCallData)
...
func (s *CrossLayerUser) ProveWithdrawal(t Testing, l2TxHash common.Hash) common.Hash {
	...
	// Create the prove tx
	tx, err := s.L1.env.Bindings.MantlePortal.ProveWithdrawalTransaction(
		&s.L1.txOpts,
...
func (s *CrossLayerUser) CompleteWithdrawal(t Testing, l2TxHash common.Hash) common.Hash {
	...
	tx, err := s.L1.env.Bindings.MantlePortal.FinalizeWithdrawalTransaction(
		&s.L1.txOpts,
		bindings.TypesWithdrawalTransaction{
			Nonce:    params.Nonce,
			Sender:   params.Sender,

...
// chain.
func (e *executor) applyTx(tx *types.Transaction, txSetProof *types.BatchTxSetProof) (error, *types.Block) {
	e.mu.RLock()
	defer e.mu.RUnlock()
	tstart := time.Now()
	...
	if err := e.engine.Prepare(e.chain, header, txSetProof); err != nil {

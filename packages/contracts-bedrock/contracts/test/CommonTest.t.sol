...
import { MantleMintableERC20Factory } from "../universal/MantleMintableERC20Factory.sol";
...
contract Portal_Initializer is L2OutputOracle_Initializer {
    ...
    function setUp() public virtual override {
        ...
        opImpl = new MantlePortal({ _l2Oracle: oracle, _guardian: guardian, _paused: true });
        Proxy proxy = new Proxy(multisig);
        vm.prank(multisig);
        proxy.upgradeToAndCall(
            address(opImpl),
            abi.encodeWithSelector(MantlePortal.initialize.selector, false)
        );
        op = MantlePortal(payable(address(proxy)));
    }
}
...
contract Messenger_Initializer is L2OutputOracle_Initializer {
    ...
    function setUp() public virtual override {
        ...
        // Deploy the MantlePortal
        op = new MantlePortal({ _l2Oracle: oracle, _guardian: guardian, _paused: false });
        vm.label(address(op), "MantlePortal");
...
contract Bridge_Initializer is Messenger_Initializer {
    ...
    MantleMintableERC20Factory L2TokenFactory;
    MantleMintableERC20Factory L1TokenFactory;
    ERC20 L1Token;
    ERC20 BadL1Token;
    MantleMintableERC20 L2Token;
    ERC20 NativeL2Token;
    ERC20 BadL2Token;
    MantleMintableERC20 RemoteL1Token;
    ...
    function setUp() public virtual override {
        ...
        proxy.setCode(address(new L1StandardBridge(payable(address(L1Messenger)),Predeploys.L1_BIT_ADDRESS)).code);
        vm.clearMockedCalls();
        ...
        BadL2Token = MantleMintableERC20(
            L2TokenFactory.createStandardL2Token(
                address(1),
                string(abi.encodePacked("L2-", L1Token.name())),
                string(abi.encodePacked("L2-", L1Token.symbol()))
        ...
        L1TokenFactory = new MantleMintableERC20Factory(address(L1Bridge));

        RemoteL1Token = MantleMintableERC20(
        ...
        BadL1Token = MantleMintableERC20(
            L1TokenFactory.createStandardL2Token(
                address(1),
                string(abi.encodePacked("L1-", NativeL2Token.name())),
                string(abi.encodePacked("L1-", NativeL2Token.symbol()))

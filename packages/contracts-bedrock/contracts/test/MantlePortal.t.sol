...
contract MantlePortal_Test is Portal_Initializer {
    ...
    /**
     * @notice The MantlePortal can be paused by the GUARDIAN
     */
    ...
    /**
     * @notice The MantlePortal reverts when an account that is not the
    ...
    function test_pause_onlyGuardian_reverts() external {
        ...
        vm.expectRevert("MantlePortal: only guardian can pause");
        vm.prank(alice);
        op.pause();
    ...
    /**
     * @notice The MantlePortal can be unpaused by the GUARDIAN
     */
    ...
    /**
     * @notice The MantlePortal reverts when an account that is not
    ...
    function test_unpause_onlyGuardian_reverts() external {
        ...
        assertTrue(op.GUARDIAN() != alice);
        vm.expectRevert("MantlePortal: only guardian can unpause");
        vm.prank(alice);
        op.unpause();
    ...
    function test_depositTransaction_contractCreation_reverts() external {
        ...
        vm.expectRevert("MantlePortal: must send to address(0) when creating a contract");
    ...
    function test_depositTransaction_smallGasLimit_reverts() external {
        vm.expectRevert("MantlePortal: gas limit must cover instrinsic gas cost");
...
contract MantlePortal_FinalizeWithdrawal_Test is Portal_Initializer {
    ...
    function callPortalAndExpectRevert() external payable {
        vm.expectRevert("MantlePortal: can only trigger one withdrawal per transaction");
    ...
    function test_proveWithdrawalTransaction_paused_reverts() external {
        ...
        vm.expectRevert("MantlePortal: paused");
    ...
    // Test: proveWithdrawalTransaction cannot prove a withdrawal with itself (the MantlePortal) as the target.
    function test_proveWithdrawalTransaction_onSelfCall_reverts() external {
        _defaultTx.target = address(op);
        vm.expectRevert("MantlePortal: you cannot send messages to the portal contract");
        op.proveWithdrawalTransaction(
            _defaultTx,
    ...
    function test_proveWithdrawalTransaction_onInvalidOutputRootProof_reverts() external {
        ...
        vm.expectRevert("MantlePortal: invalid output root proof");
        op.proveWithdrawalTransaction(
            _defaultTx,
            _proposedOutputIndex,
            _outputRootProof,
    ...
    function test_proveWithdrawalTransaction_replayProve_reverts() external {
        ...
        vm.expectRevert("MantlePortal: withdrawal hash has already been proven");
        op.proveWithdrawalTransaction(
            _defaultTx,
            _proposedOutputIndex,
            _outputRootProof,
    ...
    function test_proveWithdrawalTransaction_replayProveChangedOutputRoot_succeeds() external {
        ...
        assembly {
            ...
            mstore(0x20, 52) // 52 is the slot of the `provenWithdrawals` mapping in the MantlePortal
        ...
        external
    {
        ...
        assembly {
            mstore(0x00, sload(_withdrawalHash.slot))
            mstore(0x20, 52) // 52 is the slot of the `provenWithdrawals` mapping in MantlePortal
            slot := keccak256(0x00, 0x40)
        }
    ...
    function test_finalizeWithdrawalTransaction_ifWithdrawalNotProven_reverts() external {
        ...
        vm.expectRevert("MantlePortal: withdrawal has not been proven yet");
        op.finalizeWithdrawalTransaction(_defaultTx);
    ...
    function test_finalizeWithdrawalTransaction_ifWithdrawalProofNotOldEnough_reverts() external {
        ...
        vm.expectRevert("MantlePortal: proven withdrawal finalization period has not elapsed");
        op.finalizeWithdrawalTransaction(_defaultTx);
    ...
    function test_finalizeWithdrawalTransaction_timestampLessThanL2OracleStart_reverts() external {
        ...
        vm.expectRevert(
            "MantlePortal: withdrawal timestamp less than L2 Oracle starting timestamp"
        );
        op.finalizeWithdrawalTransaction(_defaultTx);
    ...
    function test_finalizeWithdrawalTransaction_ifOutputRootChanges_reverts() external {
        ...
        // Attempt to finalize the withdrawal
        vm.expectRevert(
            "MantlePortal: output root proven is not the same as current output root"
        );
        op.finalizeWithdrawalTransaction(_defaultTx);
    ...
    function test_finalizeWithdrawalTransaction_ifOutputTimestampIsNotFinalized_reverts() external {
        ...
        // Attempt to finalize the withdrawal
        vm.expectRevert("MantlePortal: output proposal finalization period has not elapsed");
        op.finalizeWithdrawalTransaction(_defaultTx);
    ...
    function test_finalizeWithdrawalTransaction_onReplay_reverts() external {
        ...
        vm.expectRevert("MantlePortal: withdrawal has already been finalized");
        op.finalizeWithdrawalTransaction(_defaultTx);
    }
    ...
    function test_finalizeWithdrawalTransaction_onInsufficientGas_reverts() external {
        ...
        vm.warp(block.timestamp + oracle.FINALIZATION_PERIOD_SECONDS() + 1);
        vm.expectRevert("MantlePortal: insufficient gas to finalize withdrawal");
...
contract MantlePortalUpgradeable_Test is Portal_Initializer {
    ...
    function test_params_initValuesOnProxy_succeeds() external {
        (uint128 prevBaseFee, uint64 prevBoughtGas, uint64 prevBlockNum) = MantlePortal(
    ...
    function test_initialize_cannotInitProxy_reverts() external {
        ...
        MantlePortal(payable(proxy)).initialize(false);
    }

    function test_initialize_cannotInitImpl_reverts() external {
        vm.expectRevert("Initializable: contract is already initialized");
        MantlePortal(opImpl).initialize(false);
    }

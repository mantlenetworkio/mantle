...
import (
	...
	"github.com/mantlenetworkio/mantle/mt-node/withdrawals"
)
...
func TestSystemE2E(t *testing.T) {
	...
	depositContract, err := bindings.NewMantlePortal(predeploys.DevMantlePortalAddr, l1Client)
	require.Nil(t, err)
...
func TestWithdrawals(t *testing.T) {
	...
	defer cancel()
	blockNumber, err := withdrawals.WaitForFinalizationPeriod(ctx, l1Client, predeploys.DevMantlePortalAddr, receipt.BlockNumber)
	require.Nil(t, err)
	...
	portal, err := bindings.NewMantlePortal(predeploys.DevMantlePortalAddr, l1Client)
	require.Nil(t, err)
	...
	defer cancel()
	_, err = withdrawals.WaitForFinalizationPeriod(ctx, l1Client, predeploys.DevMantlePortalAddr, header.Number)
	require.Nil(t, err)

...
func TestSequencerSetSequencerBasic(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			err: true,
			msg: "invalid sequencer #0: sequencer does not have a node id",
		},
		{
			seqs: SequencerSet{
				Sequencers: []*Sequencer{seq},
...
func TestIncrementProducerPriorityPositiveTimes(t *testing.T) {
	sset := NewSequencerSet([]*Sequencer{
		newSequencer(byteToCommonAddr([]byte("foo")), randNodeID(), 1000),
		newSequencer(byteToCommonAddr([]byte("bar")), randNodeID(), 300),
		newSequencer(byteToCommonAddr([]byte("baz")), randNodeID(), 330),
	})
...
func BenchmarkSequencerSetCopy(b *testing.B) {
	...
	for i := 0; i < 1000; i++ {
		...
		seq := NewSequencer(randAddress(), privKey.D.Bytes(), 10)
...
func TestProducerSelection2(t *testing.T) {
	...
	seq0, seq1, seq2 := newSequencer(byteToCommonAddr(addr0), randNodeID(), 100), newSequencer(byteToCommonAddr(addr1), randNodeID(), 100), newSequencer(byteToCommonAddr(addr2), randNodeID(), 100)
	...
	*seq2 = *newSequencer(byteToCommonAddr(addr2), randNodeID(), 400)
	seqs = NewSequencerSet(seqList)
	// seqs.IncrementProducerPriority(1)
	prop := seqs.GetProducer()
	...
	*seq2 = *newSequencer(byteToCommonAddr(addr2), randNodeID(), 401)
	seqs = NewSequencerSet(seqList)
	prop = seqs.GetProducer()
	if !bytes.Equal(prop.Address.Bytes(), addr2) {
		t.Fatalf("Expected address with highest voting power to be first proposer. Got %X", prop.Address)
	...
	seq0, seq1, seq2 = newSequencer(byteToCommonAddr(addr0), randNodeID(), 4), newSequencer(byteToCommonAddr(addr1), randNodeID(), 5), newSequencer(byteToCommonAddr(addr2), randNodeID(), 3)
	seqList = []*Sequencer{seq0, seq1, seq2}
...
func TestProducerSelection3(t *testing.T) {
	sset := NewSequencerSet([]*Sequencer{
		newSequencer(byteToCommonAddr([]byte("asequencer_address12")), randNodeID(), 1),
		newSequencer(byteToCommonAddr([]byte("bsequencer_address12")), randNodeID(), 1),
		newSequencer(byteToCommonAddr([]byte("csequencer_address12")), randNodeID(), 1),
		newSequencer(byteToCommonAddr([]byte("dsequencer_address12")), randNodeID(), 1),
	})
	...
	for i := 0; i < 4; i++ {
		...
		privKey, _ := ecdsa.GenerateKey(elliptic.P256(), crand.Reader)
		sset.Sequencers[i].NodeID = privKey.D.Bytes()
...
func newSequencer(address common.Address, nodeID []byte, power int64) *Sequencer {
	return &Sequencer{Address: address, NodeID: nodeID, VotingPower: power}
}
...
func TestEmptySet(t *testing.T) {
	...
	// Add to empty set
	v1 := newSequencer(byteToCommonAddr([]byte("v1")), randNodeID(), 100)
	v2 := newSequencer(byteToCommonAddr([]byte("v2")), randNodeID(), 100)
	...
	v1 = newSequencer(byteToCommonAddr([]byte("v1")), randNodeID(), 0)
	v2 = newSequencer(byteToCommonAddr([]byte("v2")), randNodeID(), 0)
...
func TestUpdatesForNewSequencerSet(t *testing.T) {
	...
	v111 := newSequencer(byteToCommonAddr([]byte("v1")), randNodeID(), 100)
	v112 := newSequencer(byteToCommonAddr([]byte("v1")), randNodeID(), 123)
	v113 := newSequencer(byteToCommonAddr([]byte("v1")), randNodeID(), 234)
	...
	v1 = newSequencer(byteToCommonAddr([]byte("v1")), randNodeID(), 0)
	v2 = newSequencer(byteToCommonAddr([]byte("v2")), randNodeID(), 22)
	v3 := newSequencer(byteToCommonAddr([]byte("v3")), randNodeID(), 33)
	seqList = []*Sequencer{v1, v2, v3}
	assert.Panics(t, func() { NewSequencerSet(seqList) })

	// Verify set including seqidator with negative voting power cannot be created
	v1 = newSequencer(byteToCommonAddr([]byte("v1")), randNodeID(), 10)
	v2 = newSequencer(byteToCommonAddr([]byte("v2")), randNodeID(), -20)
	v3 = newSequencer(byteToCommonAddr([]byte("v3")), randNodeID(), 30)
	seqList = []*Sequencer{v1, v2, v3}
	assert.Panics(t, func() { NewSequencerSet(seqList) })
}
...
func createNewSequencerList(testSeqList []testSeq) []*Sequencer {
	...
	for _, seq := range testSeqList {
		seqList = append(seqList, newSequencer(byteToCommonAddr([]byte(seq.name)), randNodeID(), seq.power))
	}
	return seqList
}
...
func BenchmarkUpdates(b *testing.B) {
	...
	vs := make([]*Sequencer, n)
	for j := 0; j < n; j++ {
		vs[j] = newSequencer(byteToCommonAddr([]byte(fmt.Sprintf("v%d", j))), randNodeID(), 100)
	}
	...
	for j := 0; j < m; j++ {
		newValList[j] = newSequencer(byteToCommonAddr([]byte(fmt.Sprintf("v%d", j+l))), randNodeID(), 1000)
	}
	b.ResetTimer()

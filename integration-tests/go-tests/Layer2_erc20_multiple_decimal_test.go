...
import (
	...
	l1mantle "github.com/tenderly/optimism/go-test/contracts/L1/local/LocalMantleToken.sol"
	l1bridge "github.com/tenderly/optimism/go-test/contracts/L1/messaging/L1StandardBridge.sol"
	l2bridge "github.com/tenderly/optimism/go-test/contracts/L2/messaging/L2StandardBridge.sol"
	factory "github.com/tenderly/optimism/go-test/contracts/L2/messaging/L2StandardTokenFactory.sol"
	l1USDT "github.com/tenderly/optimism/go-test/contracts/fiat"
...
func TestNewTokenDepositAndWithdraw(t *testing.T) {
	...
	tx, err := l1Bridge.DepositERC20(auth, common.HexToAddress(l1MantleAddress), common.HexToAddress(l2MantleAddress), big.NewInt(DECIMAL1), 2_000_000, []byte("0x"))
	require.NoError(t, err)
	...
	t.Log("l2 mantle balance: ", getMANTLEBalanceFromL2(t, userAddress))
	t.Log("mantle deposit amount: ", DECIMAL1)
	...
	setL1MantleApprove(t)
	t.Log("l1 tt balance: ", getMANTLEBalanceFromL1(t, userAddress))
	t.Log("l2 tt balance: ", getMANTLEBalanceFromL2(t, userAddress))
	auth = buildAuth(t, l1Client, userPrivateKey, big.NewInt(0))
...
func getERC20TokenFromL1(t *testing.T, address string, token string) *big.Int {
	...
	l1MantleInstance, err := l1mantle.NewL1MantleToken(common.HexToAddress(token), client)
	require.NoError(t, err)

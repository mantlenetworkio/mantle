  ...
  protected async _start(): Promise<void> {
    ...
    for (const fixHeight of fixHeights) {
      ...
      if (enqueue.data !== unConfirmedTx.data) {
        console.log(enqueue)
        enqueue.target = unConfirmedTx.target
        enqueue.data = unConfirmedTx.data
        enqueue.gasLimit = unConfirmedTx.gasLimit
        enqueue.origin = unConfirmedTx.origin
        enqueue.blockNumber = unConfirmedTx.blockNumber
        enqueue.timestamp = unConfirmedTx.timestamp

...
const createMantleMintableERC20 = async (
  hre: HardhatRuntimeEnvironment,
  L1ERC20: Contract,
  l2Signer: Wallet
): Promise<Contract> => {
  const Artifact__MantleMintableERC20Token = await getContractDefinition(
    'MantleMintableERC20'
  )
  ...
  const MantleMintableERC20TokenFactory = new Contract(
    ...
    Artifact__MantleMintableERC20TokenFactory.abi,
    l2Signer
  )
  ...
  const event = receipt.events.find(
    (e: Event) => e.event === 'MantleMintableERC20Created'
  )

  if (!event) {
    throw new Error('Unable to find MantleMintableERC20Created event')
  }
  ...
  return new Contract(
    l2WethAddress,
    Artifact__MantleMintableERC20Token.abi,
    l2Signer
  )
}
...
task('deposit-erc20', 'Deposits WETH9 onto L2.')
  ...
  .setAction(async (args, hre) => {
    ...
    const MantlePortal = new hre.ethers.Contract(
      contractAddrs.l1.MantlePortal,
      Artifact__MantlePortal.abi,
      signer
    )
    ...
    const MantleMintableERC20 = await createMantleMintableERC20(
      hre,
      WETH9,
      l2Signer
    )
    ...
    const l2Balance = await MantleMintableERC20.balanceOf(address)
    ...
    for (const log of finalizeReceipt.logs) {
      switch (log.address) {
        case MantlePortal.address: {
          const parsed = MantlePortal.interface.parseLog(log)
          console.log(`Log ${parsed.name} from MantlePortal (${log.address})`)
          console.log(parsed.args)
          console.log()
          break
        }

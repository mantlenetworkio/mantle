...
import { getChainId } from '@bitdaoio/core-utils'
import { predeploys } from '@bitdaoio/contracts'
import Artifact__TestERC721 from '@bitdaoio/contracts-periphery/artifacts/contracts/testing/helpers/TestERC721.sol/TestERC721.json'
import Artifact__L1ERC721Bridge from '@bitdaoio/contracts-periphery/artifacts/contracts/L1/L1ERC721Bridge.sol/L1ERC721Bridge.json'
import Artifact__L2ERC721Bridge from '@bitdaoio/contracts-periphery/artifacts/contracts/L2/L2ERC721Bridge.sol/L2ERC721Bridge.json'
import Artifact__BitnetworkMintableERC721Factory from '@bitdaoio/contracts-periphery/artifacts/contracts/universal/op-erc721/BitnetworkMintableERC721Factory.sol/BitnetworkMintableERC721Factory.json'
import Artifact__BitnetworkMintableERC721 from '@bitdaoio/contracts-periphery/artifacts/contracts/universal/op-erc721/BitnetworkMintableERC721.sol/BitnetworkMintableERC721.json'
...
describe('ERC721 Bridge', () => {
  ...
  let Factory__BitnetworkMintableERC721Factory: ContractFactory
  before(async () => {
    ...
    Factory__BitnetworkMintableERC721Factory = await ethers.getContractFactory(
      Artifact__BitnetworkMintableERC721Factory.abi,
      Artifact__BitnetworkMintableERC721Factory.bytecode,
      bobWalletL2
    )
  })
  ...
  let BitnetworkMintableERC721: Contract
  beforeEach(async () => {
    ...
    BitnetworkMintableERC721Factory =
      await Factory__BitnetworkMintableERC721Factory.deploy(
        L2ERC721Bridge.address,
        await getChainId(env.l1Wallet.provider)
      )
    await BitnetworkMintableERC721Factory.deployed()
    ...
    BitnetworkMintableERC721 = await ethers.getContractAt(
      Artifact__BitnetworkMintableERC721.abi,
      erc721CreatedEvent.args.localToken
    )
    await BitnetworkMintableERC721.deployed()
  ...
  it('bridgeERC721', async () => {
    ...
    expect(await BitnetworkMintableERC721.ownerOf(TOKEN_ID)).to.equal(bobAddress)
  })
  ...
  it('bridgeERC721To', async () => {
    ...
    expect(await BitnetworkMintableERC721.ownerOf(TOKEN_ID)).to.equal(
      aliceAddress
    )
  })
  ...
  withdrawalTest('bridgeERC721', async () => {
    ...
    // L2 NFT is burned
    await expect(BitnetworkMintableERC721.ownerOf(TOKEN_ID)).to.be.reverted
  })
  ...
  withdrawalTest(
    ...
    async () => {
      ...
      const FakeBitnetworkMintableERC721 = await (
        await ethers.getContractFactory(
          'FakeBitnetworkMintableERC721',
          bobWalletL2
        )
      ).deploy(L1ERC721.address, L2ERC721Bridge.address)
      await FakeBitnetworkMintableERC721.deployed()

      // Use the fake contract to mint Alice an NFT with the same token ID
      const tx = await FakeBitnetworkMintableERC721.mint(aliceAddress, TOKEN_ID)
      await tx.wait()

      // Check that Alice owns the NFT from the fake ERC721 contract
      expect(await FakeBitnetworkMintableERC721.ownerOf(TOKEN_ID)).to.equal(
        aliceAddress
      )
      ...
      const withdrawalTx = await L2ERC721Bridge.connect(
        ...
        FakeBitnetworkMintableERC721.address,
        L1ERC721.address,
        TOKEN_ID,
        0,
        NON_NULL_BYTES

...
parser = argparse.ArgumentParser(description='Configure an Mantle Bedrock replica using docker-compose.')
...
parser.add_argument('--geth-tag', type=str, help='docker tag to use with geth', default='Mantle-history')
...
parser.add_argument('--mt-node-tag', type=str, help='docker tag to use with the rollup node', default='develop')
parser.add_argument('--mt-node-http-port', type=int, help='rollup node http port', default=9545)
parser.add_argument('--mt-node-metrics-port', type=int, help='rollup node http port', default=7300)
parser.add_argument('--mt-node-pprof-port', type=int, help='rollup node http port', default=6300)
...
services:
  l2:
    image: ethereumMantle/reference-optimistic-geth:{geth_tag}
    ports:
      - "{geth_http_port}:8545"
      - "{geth_ws_port}:8546"
    volumes:

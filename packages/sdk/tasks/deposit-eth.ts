...
task('deposit-eth', 'Deposits ether to L2.')
  ...
  .setAction(async (args, hre) => {
    ...
    if (args.l1ContractsJsonPath) {
      ...
      const Deployment__MantlePortal = await hre.deployments.get(
        'MantlePortalProxy'
      )
      ...
      contractAddrs = {
        l1: {
          ...
          MantlePortal: Deployment__MantlePortal.address,
    ...
    console.log(
      `MantlePortal balance before: ${formatEther(opBalanceBefore)}`
    )
    console.log(`MantlePortal balance after: ${formatEther(opBalanceAfter)}`)

    if (!opBalanceBefore.add(amount).eq(opBalanceAfter)) {
      throw new Error(`MantlePortal balance mismatch`)
    }

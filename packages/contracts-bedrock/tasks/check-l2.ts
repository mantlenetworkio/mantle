...
const check = {
  ...
  // - check version
  MantleMintableERC20Factory: async (
    hre: HardhatRuntimeEnvironment,
    signer: Signer
  ) => {
    const MantleMintableERC20Factory = await hre.ethers.getContractAt(
      'MantleMintableERC20Factory',
    ...
    const BRIDGE = await MantleMintableERC20Factory.BRIDGE()
    assert(BRIDGE !== hre.ethers.constants.AddressZero)

    await checkProxy(hre, 'MantleMintableERC20Factory', signer.provider)
    await assertProxy(hre, 'MantleMintableERC20Factory', signer.provider)
  },
  // L1BlockNumber
  // - check version
  ...
  // - check version
  MantleMintableERC721Factory: async (
    hre: HardhatRuntimeEnvironment,
    signer: Signer
  ) => {
    const MantleMintableERC721Factory = await hre.ethers.getContractAt(
      'MantleMintableERC721Factory',
      predeploys.MantleMintableERC721Factory,
      signer
    )
    ...
    const BRIDGE = await MantleMintableERC721Factory.BRIDGE()
    assert(BRIDGE !== hre.ethers.constants.AddressZero)
    console.log(`  - BRIDGE: ${BRIDGE}`)

    const REMOTE_CHAIN_ID =
      await MantleMintableERC721Factory.REMOTE_CHAIN_ID()
    assert(REMOTE_CHAIN_ID !== 0)
    console.log(`  - REMOTE_CHAIN_ID: ${REMOTE_CHAIN_ID}`)

    await checkProxy(hre, 'MantleMintableERC721Factory', signer.provider)
    await assertProxy(hre, 'MantleMintableERC721Factory', signer.provider)
  },
  // ProxyAdmin
  // - check owner
